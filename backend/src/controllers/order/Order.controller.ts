...
import { NewOrder } from "../../types/types.js";
...
export const newOrder = TryCatch(
  async (req: Request<{}, {}, NewOrder>, res: Response, next: NextFunction) => {
    const {
      shippingInfo,
      user,
      subtotal,
...
export const myOrders = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (NodeCache.has(`allorders`)) {
      order = JSON.parse(NodeCache.get(`allorders`) as string);
    } else {
...
export const allOrders = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (NodeCache.has(`myorder-${id}`)) {
      order = JSON.parse(NodeCache.get(`myorder-${id}`) as string);
    } else {
...
export const getOrder = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const key = `order-${id}`;
