...
import { calculatePercentage } from "../../utils/base.js";
import { getInventories } from "../../utils/base.js";
import { getChartData } from "../../utils/base.js";

export const baseStats = TryCatch(async (req, res, next) => {
  let stats = {};
...
export const getPieCharts = TryCatch(async (req, res, next) => {
  ...
  if (NodeCache.has(key)) charts = JSON.parse(NodeCache.get(key) as string);
    ...
    const grossIncome: number = allOrders.reduce(
    ...
    const usersAgeGroup = {
      ...
      adult: allUsers.filter((i) => i.age >= 19 && i.age < 50).length,
