...
export const newProducts = TryCatch(async (req, res, next) => {
    let products;
...
export const allCategories = TryCatch(async (req, res, next) => {
    ...
    if (NodeCache.has("all-categories")) {
        categories = JSON.parse(NodeCache.get("all-categories"));
    }
    else {
...
export const allProducts = TryCatch(async (req, res, next) => {
    ...
    if (NodeCache.has("all-productsadmin")) {
        products = JSON.parse(NodeCache.get("all-productsadmin"));
    }
    else {
...
export const getProduct = TryCatch(async (req, res, next) => {
    ...
    if (NodeCache.has(`product-${req.params.id}`)) {
        product = JSON.parse(NodeCache.get(`product-${req.params.id}`));
    }
    else {
...
const generateProducts = async (count = 12) => {
    const products = [];
